...
| Visitor hammers the form 10 times in a minute from the same email | After the per-email rate limit is exceeded, the form shows "You've submitted a few times — please try again in a little while." | `apps/mirror/features/home/components/__tests__/waitlist-form.test.tsx` (Vitest component test — see transport note below) | FR-03, FR-08 |
...
| FR-08 | When the mutation throws `ConvexError` with `code: "RATE_LIMIT"`, the form shows an inline error with `data-testid="home.waitlist.form-error"` and copy "You've submitted a few times — please try again in a little while." The email input stays visible so the user can retry later. | p0 | Vitest component test in `apps/mirror/features/home/components/__tests__/waitlist-form.test.tsx`: mock `useMutation` to reject with `new ConvexError({ code: "RATE_LIMIT", retryAfterMs: 60000 })` and assert `home.waitlist.form-error` renders with the exact copy. Not Playwright — Convex mutations travel over WebSocket frames that `page.route` cannot intercept (see transport note in Playwright E2E Tests). |
...
Stop reason: **quality bar met** after two critique rounds + two targeted revision passes, followed by a Phase-5 verification pass that surfaced three editorial drift issues (stale rate-limit copy in the scenarios table, stale FR-08 verification column pointing at an impossible Playwright strategy, missing `test:unit` in Step 2 hard gate) — all three resolved. One verifier check (`Critique:` agents required per orchestration step) was rejected as a stale checklist item: the current `.claude/skills/create-spec/SKILL.md` and this spec's Team Orchestration Plan both explicitly state that reviewer selection lives in `.claude/skills/orchestrate-implementation/SKILL.md`, not in the spec.
