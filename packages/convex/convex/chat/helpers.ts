...
// message, so naming it proactively has the most impact. Contact follows bio
// so the agent reads "who they are → how to reach them → what they've
...
const POST_OWNER_WRITE_VOCABULARY =
  "Call editPost with the slug only when the profile owner explicitly asks to open the editor or edit a specific post — the result navigates them to the inline editor. Call deletePost with the slug only when the visitor (who must be the profile owner) explicitly asks to remove a post — never delete on your own initiative, and always confirm the slug before calling. Call publishPost with the slug only when the profile owner explicitly asks to publish one of their posts. Call unpublishPost with the slug only when the profile owner explicitly asks to unpublish one of their posts.";
