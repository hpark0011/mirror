...
2. `STYLE_RULES` — always present (chat UI renders plain text).
3. Tone clause — only if `tonePreset in TONE_PRESETS`.
4. `Bio: ${bio}` — only if truthy.
5. `personaPrompt || DEFAULT_PERSONA` — always present.
6. `Avoid discussing: ${topicsToAvoid}` — only if truthy.
7. Inventory sentence (`You can speak from this person's …`) — only if `contentInventory` populates ≥1 kind.
8. `TOOLS_VOCABULARY` — always present (every clone has the same tool surface; per-request factory only binds `profileOwnerId`).

RAG context is appended outside this function, in `actions.ts`. Order changes MUST update `chat/__tests__/helpers.test.ts` alongside (segment-count tests AND the section-order test).
...
## Agent tools (learned Wave 1 — agent-UI parity)

Files: `chat/tools.ts` (`"use node"`, `buildCloneTools(profileOwnerId)` factory) + `chat/toolQueries.ts` (V8 internal queries `queryLatestPublished`, `resolveBySlug`).

**Critical APIs**:
- `@convex-dev/agent` v0.6.0+ uses `inputSchema:` and `execute:` (Zod schemas). The legacy `args:` / `handler:` keys are **removed** and surface as a TypeScript error-message string at the call site (literal `"⚠️ 'handler' was removed in @convex-dev/agent v0.6.0..."`). Type signatures live in `node_modules/@convex-dev/agent/dist/client/createTool.d.ts`.
- `execute` receives `ToolCtx` = action context + optional `agent`, `userId`, `threadId`, `messageId`. Use `ctx.runQuery(internal.chat.toolQueries.<name>, { ... })` from inside `execute`.
- Tools attach **per-call** in `streamResponse`'s `streamArgs.tools = buildCloneTools(profileOwnerId)`. The `cloneAgent` singleton in `agent.ts` stays tools-less — the factory closes over `profileOwnerId` for cross-user isolation.

**Cross-user isolation invariant** (extends RAG rule from `.claude/rules/embeddings.md`):
- The LLM-visible `inputSchema` for every tool MUST NOT contain a `userId` (or any user identifier). The factory parameter `profileOwnerId` is the only path.
- Internal queries take `userId: v.id("users")` as a server-only arg, never reachable from a client codepath.
- Greppable invariant: `grep -nE "userId\s*:" packages/convex/convex/chat/tools.ts | grep -v profileOwnerId` MUST return zero hits.

**Test mocking trap**:
- Any test file that mocks `@convex-dev/agent` AND exercises a code path that loads `chat/tools.ts` (transitively through `chat/actions.ts`) MUST include `createTool: vi.fn((def) => def)` in the mock factory. Without it, `buildCloneTools` throws `No "createTool" export is defined on the "@convex-dev/agent" mock` at module-load. Affected: `rateLimits.test.ts`, `rag-cross-user.test.ts`, `tools.test.ts`. The identity stub is sufficient because `convex-test` doesn't actually invoke the LLM.
