...
// slug re-enables auto-derive — the trim rule must match the save path.
// Initialised from the current slug so editing the title of an existing
// article does not silently rename its URL.
export function useAutoSlug(form: UseFormReturn<ArticleMetadataFormData>) {
