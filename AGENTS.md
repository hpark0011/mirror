...
Turborepo monorepo. One Next.js 16 (App Router, React 19) app backed by a shared Convex deployment (real-time queries + Better Auth), with UI primitives in `packages/ui` and cross-app feature modules in `packages/features`.
...
- **[Optimistic updates](.claude/rules/optimistic-updates.md)** — list CRUD that mutates a query the same screen reads MUST wire `withOptimisticUpdate`; bare `useMutation(...)` is reserved for fire-and-forget mutations. Recipe + footguns (incl. the `Date.now`/`react-hooks/purity` trap).
