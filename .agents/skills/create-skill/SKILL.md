---
name: create-skill
description: Scaffolds new project skills in .Codex/skills/ using the repo's standard SKILL.md template. Use when the user asks to create, scaffold, author, or add a new skill, says "new skill", "make a skill", or wants to turn a repeated workflow into a reusable skill. Also use when auditing an existing skill against the template.
---

## Scope & non-goals

**Do NOT use for**: one-off prompts, personal memory entries (use auto-memory), automation that requires hooks (use `update-config`), or AGENTS.md edits. For auditing an existing skill against conventions, hand off to [`audit-skill`](../audit-skill/SKILL.md).

## Quick start

1. Clarify the brief (purpose, triggers, 80% workflow) **and grep `.Codex/skills/` for overlap** — stop if an existing skill already covers it.
2. `python3 .Codex/skills/create-skill/scripts/init_skill.py <skill-name>` — scaffolds from the template, enforces naming rules.
3. Fill frontmatter + body; delete any section without real content. Extract artifacts >20 lines into `{name}-template/`.
4. `python3 .Codex/skills/create-skill/scripts/validate_skill.py .Codex/skills/<skill-name>` — must exit 0.
...
| ------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------- |
| [scripts/init_skill.py](scripts/init_skill.py)         | Scaffold a new skill from `skill-template/SKILL.md`. Validates name against `^[a-z0-9-]+$`, ≤64 chars, no reserved words.                                                       | `init_skill.py <name> [--path .Codex/skills]` |
...
5. **References stay one level deep from SKILL.md.** Codex partially-reads nested files and loses info.
...
9. **Don't explain what Codex already knows.** Every paragraph must justify its token cost.

## Workflow

```
- [ ] 1. Clarify brief (purpose, triggers, 80% workflow, scope boundaries)
- [ ] 2. Overlap check — grep .Codex/skills/*/SKILL.md for trigger phrases
...
✗ Bad: `helper`, `utils`, `Codex-tools`, `tickets-v2`, `TicketCreator`
...
**Process:** Copy [skill-template/SKILL.md](skill-template/SKILL.md) verbatim, then fill in placeholders. Delete any section that has no real content — empty sections train Codex to skim. Extract any artifact longer than ~20 lines into `{artifact-name}-template/` and reference it.
...
✓ Good: `Created .Codex/skills/generating-commit-messages/SKILL.md — triggers on "commit this" and drafts Conventional Commit messages from staged diffs.`
...
- **Filling every section because the template has it.** Empty sections train Codex to skim. Delete them.
...
- **Nested references** (`SKILL.md → advanced.md → details.md`). Codex partial-reads these and misses content. Flat-link from SKILL.md.
...
- **Skill that duplicates AGENTS.md rules.** If it's always-on guidance, it belongs in AGENTS.md or `.Codex/rules/`, not a skill.
