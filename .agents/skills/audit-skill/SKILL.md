---
name: audit-skill
argument-hint: "[skill-name or .Codex/skills/<skill-name>]"
description: Audits an existing skill in .Codex/skills/ against the conventions enforced by create-skill — frontmatter shape, naming, section hygiene, artifact placement, and validator compliance. Use when the user says "audit this skill", "review the X skill", "check if this skill follows conventions", "maintain skill", or wants to verify an existing SKILL.md before merging changes. Produces a findings report with concrete fixes, then optionally applies them.
---

## Scope & non-goals

**Do NOT use for**: creating new skills (use `create-skill`), editing AGENTS.md rules, or mass-renaming existing skills (explicitly disallowed by `create-skill` authoring rules).

## Quick start

1. `python3 .Codex/skills/create-skill/scripts/validate_skill.py .Codex/skills/<skill-name>` — mechanical checks first.
...
- [ ] `name` is `^[a-z0-9-]+$`, ≤64 chars, no reserved words (`anthropic`, `Codex`).
...
- [ ] Not a near-duplicate of an existing skill — grep `.Codex/skills/*/SKILL.md` for the trigger phrases.
...
- [ ] Does not duplicate AGENTS.md / `.Codex/rules/` content.
...
- [ ] Prompts for sub-agents that only exist to run this skill's workflow live in `agents/<role>.md`. Sub-agents already registered in `.Codex/agents/` must be referenced by name, never copied into the skill.
...
```
audit-skill findings for .Codex/skills/foo-bar:
